...
export const Navbar = ({active,setShowRequest }: { active: string,setShowRequest?:Dispatch<SetStateAction<boolean>>}) => {
  ...
  useEffect(() => {
    ...
    const fetchMessage=async()=>{
      ...
      if(fetch?.newMessagesForNav.ids&&fetch?.newMessagesForNav.ids.length>0){
  ...
  useEffect(()=>{
    socket?.on('addMessageCount',(data:{id:string,name:string})=>{
      ...
      if(isDuplicate){
        ...
        setNewMessage(prev=>prev?.map(elem=>{
          if(elem.userId===data.id){
            return {...elem,count:elem.count>=99?99:elem.count+1}
          }else{
            return elem
          }
        }))
      }else{
        setNewMessage(prev=>([...(prev||[]),{count:1,name:data.name,userId:data.id}]))
      }
      ...
      {menuOpen && (
          <div className="fixed top-20  left-4 rounded-2xl w-[50%] border-l border-r border-b bg-gray-400 z-[1] text-black p-4 lg:hidden">
            <ul className="flex flex-col items-start w-full h-full  ">
              ...
              {(userData.subscriptionStatus!=="Not subscribed")&&<li className="hover:text-blue-500 cursor-pointer py-2 text-sm inline-flex" onClick={()=>(toggleMenu(),navigate('/planAndRequest'))}>
              Plan Datails
              
              </li>}
              {(active==='profile')&& <li className="hover:text-blue-500 cursor-pointer py-2 text-sm inline-flex" onClick={handleRequestOption}>
              Requests 
              </li>}
        ...
        {messageTab && (
          ...
          <div className="fixed top-20  right-4 rounded-2xl sm:w-[400px] w-[300px] cursor-pointer  justify-evenly  border-l max-h-52 overflow-y-auto border-r border-b bg-white z-[1] text-black p-4 ">
            ...
            {newMessage?.map((el)=>{
              return(
            <div onClick={()=>goToChat(el.userId)} className="w-full h-8 bg-blue-200 rounded-full justify-between flex items-center px-3 text-blue-600  mb-3">
              <p>{el.name}</p>
