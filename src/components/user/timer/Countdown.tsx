...
export const Countdown: React.FC<ITimerProbs> = ({
  expiryTimeStamp,
  from,
  email,
}) => {
      ...
      if (signupFirstData.EMAIL) {
        try {
          await request({
            url: "/user/otpCreation",
            method: "post",
            data: { email: signupFirstData.EMAIL, from: "signup" },
          });
        } catch (error: unknown) {
          if (error instanceof Error) {
            handleAlert("error", error.message || "internal server error");
  ...
  const singup = (
    <p className="font-sans font-semibold cursor-pointer text-white mt-1">
      ...
      <span className="pl-14" onClick={() => handleRestart("signup")}>
        Resent
      </span>
    </p>
  );
  const forgot = (
    <p className="text-white font-aborato ">
      {minutes}:{seconds}{" "}
      <span
        className="pl-24 cursor-pointer"
        onClick={() => handleRestart("forgot")}
      >
        RESET
      </span>
    </p>
