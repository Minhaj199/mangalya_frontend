...
export const credential_validation = async (
  ...
  if (
    ...
    formData["FIRST NAME"].length > 10 ||
    formData["FIRST NAME"].length < 3
  ) {
    setWarnning((el) => ({
      ...el,
      ["FIRST NAME"]: "Charector should b/w 5-10",
    }));
    return false;
  }
  if (
    formData["SECOND NAME"]?.trim() === "" ||
    formData["SECOND NAME"] === undefined
  ) {
    setWarnning((el) => ({ ...el, ["SECOND NAME"]: "Blank not allowed" }));
    return false;
  } else if (
    formData["SECOND NAME"].length < 1 ||
    formData["SECOND NAME"].length > 10
  ) {
    setWarnning((el) => ({
      ...el,
      ["SECOND NAME"]: "Charector should b/w 1-10",
    }));
    return false;
  } else {
  ...
  if (formData["DATE OF BIRTH"]) {
    ...
    if (age < 18 || age > 60) {
      setWarnning((el) => ({
        ...el,
        ["DATE OF BIRTH"]: "Age must be between 18-60",
      }));
      return false;
    }
  } else {
  ...
  if (
    formData["DISTRICT THAT YOU LIVE"]?.trim() === "" ||
    formData["DISTRICT THAT YOU LIVE"] === undefined
  ) {
    setWarnning((el) => ({
      ...el,
      ["DISTRICT THAT YOU LIVE"]: "please chouse an option",
    }));
    return false;
  ...
  if (
    formData["YOUR GENDER"]?.trim() === "" ||
    formData["YOUR GENDER"] == undefined
  ) {
    setWarnning((el) => ({
      ...el,
      ["YOUR GENDER"]: "please chouse an option",
    }));
    return false;
  } else if (formData) {
  ...
  if (
    formData["GENDER OF PARTNER"]?.trim() === "" ||
    formData["GENDER OF PARTNER"] === undefined
  ) {
    setWarnning((el) => ({
      ...el,
      ["GENDER OF PARTNER"]: "please chouse an option",
    }));
    return false;
  ...
  if (formData["EMAIL"]) {
    const isExist: { email: string | null } = await request({
  ...
  if (
    ...
    setWarnning((el) => ({
      ...el,
      ["CONFIRM PASSWORD"]: "passowrd not matching",
    }));
    return false;
  }
