      ...
      async function Purchase() {
        try {
          const response: { status: boolean; message: string } = await request({
            url: "/user/purchasePlan",
            method: "post",
            data: { planData: planInfo, token },
          });
          ...
          if (response.status === true) {
            setTimeout(() => {
              diptach({
                type: "SET_DATA",
                payload: { ...userData, subscriptionStatus: "subscribed" },
              });
              navigate("/");
          ...
          if (error instanceof Error) {
            alertWithOk(
              "Plan Purchase",
              error.message || "Error on purchase",
              "warning"
            );
          }
          console.error(error);
    ...
    <>
      ...
      <div className="h-svh w-screen flex items-center flex-col bg-blue-400 ">
        ...
        <div className="w-screen h-14 flex justify-end items-center px-2 ">
          <p className="text-white cursor-pointer" onClick={handleSkip}>
            {"DO IT LATER>"}
          </p>
        </div>
        <div
