    ...
    async function fetchPlanData() {
      try {
        ...
        if (dataDB.message && typeof dataDB.message === "string") {
          alertWithOk(
            "Validation",
            dataDB.message || "Validation faild",
            "info"
          );
          navigate("/login");
        }
      ...
      <div className="w-full h-full lg:mt-0 mt-10  overflow-hidden ">
        <div className="w-full h-[25%]  flex justify-center items-center">
          ...
          <div
            className={
              toggle
                ? " w-[90%] h-5/6 drop-shadow-lg bg-white transform transition-transform duration-300 ease-in-out hover:scale-105 rounded-lg flex justify-between items-center"
                : " w-[90%] h-5/6 drop-shadow-lg bg-white  rounded-lg flex justify-between items-center"
            }
          >
                    ...
                    >
                      ...
                      <div className=" w-[100%]  rounded-xl h-[90%] font-inter     text-white text-lg">
                        <div className="flex justify-center w-full  h-[15%] ">
                          <p className="sm:text-2xl text-base font-bold">
                            {el.name}
                          </p>
                        </div>
                        <div className="flex justify-center w-full   h-[20%]">
                          <div className="h-full w-1/3  text-center pt-1 sm:text-base text-[9px]">
                            <span className="block sm:text-sm text-[10px]">
                              Duration
                            </span>
                            {el.duration}
                          </div>
                        ...
                        <div className="w-full sm:h-36 h-32 text-sm ">
                          <ul className="p-2 flex flex-col h-full justify-around  items-center sm:text-[15px] text-[10px]">
                            {el.features.map((elem, indexOfFeatur) => (
                              <li key={indexOfFeatur}>{elem}</li>
                            ))}
                          </ul>
                        </div>
                        <div className=" w-full h-[15%] flex justify-center items-end  ">
                          <button
                            onClick={() => manageBtwAddEdit("Edit", el._id)}
                            className="bg-white text-blue-600 w-16 rounded-full text-sm font-serif"
                          >
                            EDIT
                          </button>
                        </div>
          ...
          {!toggle && currentData && (
            <div className="sm:w-[60%] bg w-[95%] sm:h-[98%] md:w-[40%]  h-[90%] rounded-3xl shadow-3xl shadow-theme-blue border-b-2 bg-white sm:px-10 px-3 items-center flex flex-col">
              <div className="h-full w-full justify-center flex flex-col items-center">
                <h1 className="font-bold text-2xl text-theme-blue mt-2 mb-5">
                  EDIT
                </h1>
                <div className="w-[100%] h-[15%] justify-between mb-2">
                  <label
                    htmlFor="name"
                    className="block font-inter font-bold text-dark-blue"
                  >
                    NAME
                  </label>
                  <input
                  ...
                  <div className="w-[33%] h-full">
                    ...
                    <input
                      onChange={(t) =>
                        setEditData((el) => ({
                          ...el,
                          amount: parseInt(t.target.value) || 0,
                        }))
                      }
                      id="amount"
                  ...
                  <div className="w-[33%] h-full ">
                    ...
                    <input
                      ...
                      onChange={(t) =>
                        setEditData((el) => ({
                          ...el,
                          connect: parseInt(t.target.value) || 0,
                        }))
                      }
                      type="number"
                  ...
                  <div className="w-[32%] h-full ">
                    ...
                    <select
                      ...
                      onChange={(t) =>
                        setEditData((el) => ({
                          ...el,
                          duration: parseInt(t.target.value) || 0,
                        }))
                      }
                ...
                <div className="w-[100%] h-[30%] bg-gray-400">
                  {editData.features.map((el, index) => (
                    <div
                      key={index}
                      className="w-full h-[20%] mt-2 bg-theme-blue flex"
                    >
                      ...
                      <div
                        ...
                        className="w-[10%] h-full text-center cursor-pointer text-white font-black"
                      >
                        X
                      </div>
                    </div>
                ...
                <button
                  ...
                  className="bg-blue-500 hover:bg-dark-blue mt-2 px-8 py-2 rounded-lg font-medium text-white "
                >
                  SUBMIT
                </button>
              </div>
